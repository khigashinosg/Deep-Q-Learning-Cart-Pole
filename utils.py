...
class ReplayBuffer():
    ...
    def sample(self, batch_size:int)->list:
        ...
        """
        batch_size = min(batch_size, len(self.buffer)) # accounts for if batch_size is larger than current buffer capacity
    
        # Use random.sample() with the updated batch_size value
        sample_result = random.sample(self.buffer, batch_size)
...
def epsilon_decay(max_e, min_e, decay_length, episode_number):
    """Calculate linearly decaying epsilon value for that episode
    
    Args:
        max: policy DQN
        min: target DQN
        decay_length: over how many episodes the epsilon should decay
        epsiode_number: current episode for which the epsilon is being calculated for
    
    Returns:
        Float scalar
    """
    decay_rate = (max_e - min_e)/decay_length # rate of decay per episode 
    decayed_epsilon = max_e - (episode_number/decay_rate) # decayed epsilon after x episodes (may be lower than allowed)
    epsilon = max(decayed_epsilon, min_e)  # if decayed_epsilon is smaller than minimum value for epsilon, return minimum value
